    ...
    loop {
        ...
        match tcp_infos {
            Ok(stats) => {
                ...
                storage.ss_bytes_retrans.push(now, stats.tcpi_bytes_retrans);
                storage.ss_bytes_acked.push(now, stats.tcpi_bytes_acked);
                storage
                    .ss_delivery_rate
                    .push(now, stats.tcpi_delivery_rate as u64);
